...
export default class HomeScreen extends React.Component {
  ...
  async componentDidMount() {
    ...
    if ((await this.isFirstLaunch()) == null) {
      ...
      logger.info(
        codeFileName,
        "componentDidMount",
        "First time app launch. Trying to set flag."
      );
      try {
        ...
        logger.info(
          codeFileName,
          "componentDidMount",
          "Setting installation date:" + _installationDate.toString()
        );
      ...
      logger.info(
        codeFileName,
        "componentDidMount",
        "Setting noSurveyDialogVisible=true."
      );
      this.setState({ noSurveyDialogVisible: true });
    } else {
      ...
      // Check if study period has ended
      {
        ...
        if (_installationDate == null) {
          ...
          _currDate = new Date();
          _diffDays = Math.round(
            Math.abs(
              (_currDate.getTime() - _installationDate.getTime()) / _oneDay
            )
          );
          if (_diffDays > _appStatus.StudyDuration) {
            ...
            logger.info(
              codeFileName,
              "componentDidMount",
              "Still in survey period. " + _diffDays + " days have passed."
            );
          }
        }
      }
        ...
        RNFS.readFile(USER_SETTINGS_FILE_PATH)
          .then(_fileContent => {
            logger.info(
              codeFileName,
              "componentDidMount",
              "Successfully read user settings file. Checking if home wifi was set."
            );
  ...
  componentWillUnmount() {
    ...
    logger.info(
      codeFileName,
      "componentWillUnmount",
      "Removing listeners and evens handlers"
    );
  }
}
