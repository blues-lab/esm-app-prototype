  ...
  componentDidMount() {
    ...
    if (_appStatus.SurveyStatus == SURVEY_STATUS.ONGOING) {
      logger.info(
        codeFileName,
        "componentDidMount",
        "Survey status is ONGOING so setting up toolbar to show remaining time."
      );
      ...
      logger.info(
        codeFileName,
        "componentDidMount",
        "No survey is ONGOING. Returning."
      );
    }
  }
      ...
      if (_minRemaining > 0) {
        ...
        if (appStatus.getStatus().SurveyStatus == SURVEY_STATUS.ONGOING) {
          ...
          logger.info(
            codeFileName,
            "updateTimeDisplay",
            "Survey expired, going back to home screen."
          );
