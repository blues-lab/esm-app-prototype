      ...
      .then(exists => {
        if (exists) {
          ...
          logger.info(
            `${codeFileName}`,
            "constructor",
            "User settings file does not exist to read file."
          );
        }
      })
      .catch(error => {
            ...
            wifi.getSSID(ssid => {
              ...
              if (ssid.length > 0) {
                logger.info(
                  codeFileName,
                  "getHomeWiFi",
                  "WiFi is connected. Asking for Home WiFi name."
                );
                Alert.alert(
                  "Home WiFi",
                  'We will only send surveys when you are connected to the home WiFi. Is "' +
                    ssid +
                    '" your home wifi?',
                  [
                    {
                      ...
                      onPress: () => {
                        Alert.alert(
                          "We'll try to ask again, when you connect to another network"
                        );
                        logger.info(
                          codeFileName,
                          "getHomeWiFi",
                    ...
                    },
                    {
                      ...
                      onPress: () => {
                        logger.info(
                          codeFileName,
                          "getHomeWiFi",
                          "Connected to home WiFi. Saving home WiFi:" + ssid
                        );
                        this.setState({ homeWifi: ssid }, () =>
    ...
    return (
      ...
      >
        <Text style={{ margin: 5, fontSize: 20 }}>
          If there is specific time of the day you do not want to receive
          surveys, while connected to the home WiFi, please indicate it below.
        </Text>
