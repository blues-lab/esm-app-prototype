...
export default class ServiceMenuScreen extends React.Component {
  ...
  createNewService(categoryName, newServiceName) {
    ...
    for (var i = 0; i < _serviceCategoriesJS.serviceCategories.length; i++) {
      if (
        _serviceCategoriesJS.serviceCategories[i].categoryName == categoryName
      ) {
  ...
  savePermissionResponse(response) {
    ...
    if (this.state.permissionPageIdx + 1 < this.state.permissionPages.length) {
    ...
    } // otherwise, go to the contextual question page
    else {
      ...
      this.setState(
        { permissionModalVisible: false, _surveyResponseJS: _surveyResponseJS },
        () =>
  ...
  showPermissionPage() {
    ...
    if (_permissionPageIdx < _permissionPages.length) {
      this.setState({
        permissionPageIdx: _permissionPageIdx,
        activeServiceCategoryName:
          _permissionPages[_permissionPageIdx].categoryName,
        activeServiceName: _permissionPages[_permissionPageIdx].serviceName,
        permissionModalVisible: false,
        surveyProgress:
          this.state.surveyProgress + Math.floor(40 / _permissionPages.length)
      });
            ...
            onPress={() => {
              ...
              if (
                ...
                logger.info(
                  "ServiceMenu",
                  "SaveButton.onPress",
                  "Navigating to contextual question page."
                );
                this.props.navigation.navigate("ContextualQuestion", {
                  surveyResponseJS: this.state.surveyResponseJS,
                  surveyProgress: 80
