...
export default class ContextualQuestionScreen extends React.Component {
  ...
  relationSelectionHandler(selectedRelation, checked) {
    if (checked) {
      this.setState({
        selectedRelations: this.state.selectedRelations.add(selectedRelation)
      });
    } else {
  ...
  locationSelectionHandler(selectedLocation, checked) {
    if (checked) {
      this.setState({
        selectedLocations: this.state.selectedLocations.add(selectedLocation)
      });
    } else {
  ...
  async saveResponse() {
    ...
    utilities.writeJSONFile(
      _surveyResponseJS,
      RNFS.DocumentDirectoryPath + "/response-" + time + ".js",
      codeFileName,
      "saveResponse"
    );
    ...
    logger.info(
      codeFileName,
      "saveResponse",
      'Updating survey status to "Completed" and removing all notifications.'
    );
    ...
    logger.info(
      codeFileName,
      "saveResponse",
      "Updating completed survey count."
    );
              ...
              <View style={styles.verticalViewStyle}>
                ...
                {this.state.numOfPeople > 0 && (
                  <View style={styles.verticalViewStyle}>
                    ...
                    <Text style={commonStyle.questionStyle}>
                      Did any of the people who were talking call in or connect
                      to a video chat (as opposed to being physically present in
                      the room)?
                    </Text>
                    <View style={styles.horizontalViewStyle}>
                      <Text>No</Text>
